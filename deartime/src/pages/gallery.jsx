...
const Gallery = () => {
  ...
  const fileInputRef = useRef(null); // 파일 업로드창 트리거용
  const longPressTimerRef = useRef(null); // 롱프레스 시간 측정용 타이머
  const isLongPressActive = useRef(false); // 클릭과 롱프레스 동작을 구분하기 위한 플래그
  ...
  // --- [UI 인터랙션 상태 관리] ---
  const [menu, setMenu] = useState({ show: false, x: 0, y: 0, targetId: null, type: null }); // 커스텀 컨텍스트 메뉴 상태
  const [editingId, setEditingId] = useState(null); // 인라인 수정을 위한 아이템 ID
  const [isModalOpen, setIsModalOpen] = useState(false); // 앨범 생성 모달 상태

  // --- [전역 이벤트 핸들러] ---
  // 메뉴 외부 클릭 시 컨텍스트 메뉴를 닫기 위한 전역 리스너
  useEffect(() => {
  ...
  const startPress = (e, id, type) => {
    if (e.type === 'mousedown' && e.button !== 0) return; // 마우스는 왼쪽 클릭만 감지
    ...
    longPressTimerRef.current = setTimeout(() => {
      ...
      isLongPressActive.current = true; // 롱프레스 상태임을 기록하여 직후 클릭 이벤트 방지
      if (navigator.vibrate) navigator.vibrate(50); // 모바일 기기 진동 피드백
    }, 500); // 0.5초 대기
  };
  ...
  const handleItemClick = (e, album = null) => {
    // 롱프레스로 메뉴를 띄운 직후 손을 뗄 때 클릭이 발생하는 것을 차단
  ...
  const handleFileUpload = (e) => {
    ...
    if (file) {
      ...
      const newPhoto = { id: Date.now(), url: imageUrl, date: today, title: file.name.split('.')[0], isFavorite: false };
  ...
  return (
    <div className="gallery-container">
      ...
      {/* 컨텍스트 메뉴 활성화 시 배경 딤 처리를 위한 오버레이 */}
      ...
      {/* 메인 콘텐츠 영역 (선택된 탭에 따른 분기 렌더링) */}
      <div className="gallery-content-wrapper">
        {activeIndex === 0 ? (
          /* RECORD 탭: 날짜별 사진 그룹 리스트 */
